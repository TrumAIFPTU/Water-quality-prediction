...
def evaluate_model(model, loader, device, split_info):
    ...
    """
    model.eval()
    preds_list, actuals_list, events_list = [], [], []
    ...
    with torch.no_grad():
        for bx, by, bevent, _ in loader:
            ...
            actuals_list.append(by_real.cpu().numpy())
            if bevent.dim() == 3:
                events_list.append(bevent.cpu().numpy())
            else:
                events_list.append(bevent.unsqueeze(-1).cpu().numpy())
                    
            
    if len(preds_list) == 0: return {}
    preds_total = np.concatenate(preds_list, axis=0)
    actuals_total = np.concatenate(actuals_list, axis=0)
    events_total = np.concatenate(events_list, axis=0)

    actuals_total = actuals_total.astype(np.float64)
    preds_total = preds_total.astype(np.float64)
